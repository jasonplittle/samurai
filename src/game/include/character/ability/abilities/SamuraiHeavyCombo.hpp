...
class SamuraiHeavyCombo : public IAbility
{
public:
    ...
    {
        std::cout << "Samurai heavy ability" << std::endl;

        m_isActive = true;
        c.Animator().Play(Animation::HeavyAttack1);
    ...
    {
        if (m_attackPhase == 1 && c.Animator().IsAfterFrame(3))
        {
    ...
    void Update(Character& c, float dt) override
    {
        ...
        if (c.Animator().IsFinished())
        {
            ...
            if (m_attackPhase == 2)
            {
                c.Animator().Play(Animation::HeavyAttack2);
