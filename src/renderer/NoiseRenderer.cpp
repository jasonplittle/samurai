...
NoiseRenderer::NoiseRenderer()
{
    ...
    m_shader = std::make_unique<Shader>("src/renderer/shaders/Noise.shader");
    ...
    ) const
{
    ...
    m_shader->SetUniform2f("u_UVScale", scale.x, scale.y);
    m_shader->SetUniform2f("u_UVOffset", offset.x, offset.y);
